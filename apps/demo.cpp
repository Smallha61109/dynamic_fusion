...
struct DynamicFusionApp {
  static void KeyboardCallback(const cv::viz::KeyboardEvent& event,
    ...
    if (event.code == 't' || event.code == 'T') kinfu.show_warp(*kinfu.kinfu_);

    if (event.code == 'i' || event.code == 'I')
      kinfu.interactive_mode_ = !kinfu.interactive_mode_;
  }

  DynamicFusionApp(std::string dir, bool visualize)
      : exit_(false),
  ...
  bool execute() {
    ...
    for (int i = 0; i < depths.size() && !exit_ && !viz.wasStopped(); i++) {
      ...
      if (has_image) show_raycasted(dynamic_fusion);
...
int main(int argc, char* argv[]) {
  ...
  std::cout << std::boolalpha << visualize << std::endl;
  ...
  if (argc == 3)
    visualize = argv[2];
  std::cout << std::boolalpha << visualize << std::endl;
  app = new DynamicFusionApp(argv[1], visualize);
  std::cout << "1" << std::endl;
