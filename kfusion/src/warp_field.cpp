  ...
  for (size_t i = 0; i < first_frame.rows; i += step)
    for (size_t j = 0; j < first_frame.cols; j += step) {
      ...
      if (!std::isnan(point.x)) {
        ...
        // utils::DualQuaternion<float>(t, utils::Quaternion<float>());
...
void WarpField::energy_data(const std::vector<Vec3d>& canonical_vertices,
  ...
  for (int i = 0; i < live_vertices.size(); i++) {
    if (std::isnan(canonical_vertices[i][0])) continue;
    getWeightsAndUpdateKNN(canonical_vertices[i],
                           weights);  // [Minhui 2018/1/28]would update the
                                      // weights and ret_index_(might be index
                                      // of KNN in nodes_)
...
void WarpField::energy_reg(const std::vector<cv::Vec3d>& surface_points,
  ...
  for (int i = 0; i < surface_points.size(); ++i) {
    if(std::isnan(surface_points[i][0]))
      continue;
  ...
  std::cout << "Adding ceres options..." << std::endl;
  ceres::Solver::Options options;
  options.linear_solver_type = ceres::SPARSE_SCHUR;
  options.minimizer_progress_to_stdout = true;
  options.num_linear_solver_threads = 12;
...
void WarpField::energy_reg(const std::vector<cv::Vec3f>& surface_points,
                           const cv::Matx33f live_camera_rot,
                           const cv::Vec3f live_camera_trans,
                           const std::vector<cv::Matx33f>& warp_rot,
                           const std::vector<cv::Vec3f>& warp_trans) {
  // TODO: maybe Tic and Tjc can be computed by inverse camera transform *
  // warp?
...
// void WarpField::energy_reg(const
// std::vector<std::pair<kfusion::utils::DualQuaternion<float>,
...
// (broken)
// void WarpField::updateTransformations(double[KNN_NEIGHBOURS][6]
// transformations) {
//     for(int i = 0; i < KNN_NEIGHBOURS; i++) {
//         nodes_->at(ret_index_[i]).transform.encodeRotation(transformations[i][0],
//         transformations[i][1], transformations[i][2]);
//         nodes_->at(ret_index_[i]).transform.encodeTranslation(transformations[i][3],
//         transformations[i][4], transformations[i][5]);
//         //TODO: (check) The function encodeTranslation() is not compatible
//         with getTranslation() [not sure]
//     }
// }
