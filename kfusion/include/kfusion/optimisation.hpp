...
struct DynamicFusionDataEnergy {
  ...
  T tukeyPenalty(T x, T c = T(0.01)) const {
    /* TODO: this seems to mean that 0.01 is the acceptable threshold for x
             (otherwise return 0 and as such, it converges). Need to check if
             this is correct */
  ...
  static ceres::CostFunction *Create(
      ...
      const int index) {
    auto cost_function =
        new ceres::DynamicNumericDiffCostFunction<DynamicFusionDataEnergy>(
...
struct DynamicFusionRegEnergy {
  ...
  virtual ~DynamicFusionRegEnergy() { delete[] weights_; };
  ...
  static ceres::CostFunction *Create(
      const std::vector<kfusion::deformation_node> &nodes,
      const std::vector<size_t> &ret_index,
      const float weights[KNN_NEIGHBOURS], const cv::Affine3f &inverse_pose) {
    auto j_point = nodes.at(ret_index[0]).vertex;
    auto cost_function =
        new ceres::DynamicNumericDiffCostFunction<DynamicFusionRegEnergy>(
            new DynamicFusionRegEnergy(nodes, ret_index, weights, inverse_pose));
    for (int i = 0; i < KNN_NEIGHBOURS; i++)
      cost_function->AddParameterBlock(8);
    cost_function->SetNumResiduals(1);
    return cost_function;
 ...
 public:
  explicit WarpProblem(kfusion::WarpField *warp) : warpField_(warp) {
    ...
    for (int i = 0; i < warpField_->getNodes()->size(); i++) {
      parameters_[i * 8 + 0] =
          warpField_->getNodes()->at(i).transform.rotation_.w_;
      parameters_[i * 8 + 1] =
      ...
      parameters_[i * 8 + 4] =
          warpField_->getNodes()->at(i).transform.translation_.w_;
      parameters_[i * 8 + 5] =
  ...
  ~WarpProblem() { delete[] parameters_; }
  ...
  const double *params() const { return parameters_; }
